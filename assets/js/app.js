document.addEventListener("DOMContentLoaded", function () {
    ...
    function (entries) {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          entry.target.classList.remove("fade-in-fast");
        }
      });
    },
    { threshold: 0.1 }
  ...
  // Observar todos los elementos que deseas animar
...
$(document).ready(function () {
  ...
  $(".advertice").addClass("fade-in-advertice");
  $("#duck-button").mouseenter(start).mouseleave(stop);
  $(document).ready(function () {
    $(".corner-duck")
      .mouseenter(function () {
        $(".corner-duck-message").addClass("visible");
      })
      .mouseleave(function () {
        $(".corner-duck-message").removeClass("visible");
      });
  });

  $(".corner-duck").click(function () {
    $(".corner-duck-hearts").addClass("visible");
    $(".corner-duck-message").html(
      "A mi me gusta! <br><strong>Te quiero</strong>"
    );
    setTimeout(function () {
      $(".corner-duck-hearts").removeClass("visible");
      $(".corner-duck-message").html("A mí no! <br>el otro pato");
    ...
    spirals.forEach(function (spiral) {
      ...
      for (var i = 0; i < spiral.getTotalLength(); i += 1) {
        ...
        if (length < spiral.getTotalLength()) {
          ...
          const deg =
            (Math.atan2(point.y - before.y, point.x - before.x) * 180) /
            Math.PI;
          var leaf = g.appendChild(
